...
module DataMemReadWrap(	// @[<stdin>:791:10]
  ...
  assign io_readData =
    io_control_memOp == 3'h1
      ...
      : io_control_memOp == 3'h3 ? {{16{shiftData[15]}}, shiftData[15:0]} : shiftData;	// @[<stdin>:791:10, src/main/scala/chisel3/util/Mux.scala:141:16, src/src/core/MEM/DataMemWrap.scala:69:31, :73:{22,40,45,59,74}, :74:{22,40,45,59,75}]
endmodule

module MEM(	// @[<stdin>:821:10]
  ...
  always @(posedge clock) begin	// @[<stdin>:822:11]
    if (reset) begin	// @[<stdin>:822:11]
      ...
      exe2mem_iddata_control_memOp <= 3'h0;	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:31:24]
  ...
  `ifdef ENABLE_INITIAL_REG_	// @[<stdin>:821:10]
    `ifdef FIRRTL_BEFORE_INITIAL	// @[<stdin>:821:10]
      `FIRRTL_BEFORE_INITIAL	// @[<stdin>:821:10]
    `endif // FIRRTL_BEFORE_INITIAL
    logic [31:0] _RANDOM[0:6];	// @[<stdin>:821:10]
    initial begin	// @[<stdin>:821:10]
      `ifdef INIT_RANDOM_PROLOG_	// @[<stdin>:821:10]
        `INIT_RANDOM_PROLOG_	// @[<stdin>:821:10]
      `endif // INIT_RANDOM_PROLOG_
      `ifdef RANDOMIZE_REG_INIT	// @[<stdin>:821:10]
        for (logic [2:0] i = 3'h0; i < 3'h7; i += 3'h1) begin
          _RANDOM[i] = `RANDOM;	// @[<stdin>:821:10]
        end	// @[<stdin>:821:10]
        wbValid = _RANDOM[3'h0][0];	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:21:27]
        exe2mem_ifdata_pc = {_RANDOM[3'h0][31:1], _RANDOM[3'h1][0]};	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:21:27, :31:24]
        exe2mem_ifdata_inst = {_RANDOM[3'h1][31:1], _RANDOM[3'h2][0]};	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:31:24]
        exe2mem_iddata_control_memOp = _RANDOM[3'h2][18:16];	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:31:24]
        exe2mem_iddata_control_wbOp = _RANDOM[3'h2][22:21];	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:31:24]
        exe2mem_iddata_control_halt = _RANDOM[3'h2][23];	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:31:24]
        exe2mem_exedata_aluResult = {_RANDOM[3'h5][31:24], _RANDOM[3'h6][23:0]};	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:31:24]
      `endif // RANDOMIZE_REG_INIT
    end // initial
    `ifdef FIRRTL_AFTER_INITIAL	// @[<stdin>:821:10]
      `FIRRTL_AFTER_INITIAL	// @[<stdin>:821:10]
    `endif // FIRRTL_AFTER_INITIAL
  `endif // ENABLE_INITIAL_REG_
  ...
  assign io_mem2wb_valid = wbValid;	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:21:27]
  assign io_mem2wb_bits_ifdata_pc = exe2mem_ifdata_pc;	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:31:24]
  assign io_mem2wb_bits_ifdata_inst = exe2mem_ifdata_inst;	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:31:24]
  assign io_mem2wb_bits_iddata_control_wbOp = exe2mem_iddata_control_wbOp;	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:31:24]
  assign io_mem2wb_bits_iddata_control_halt = exe2mem_iddata_control_halt;	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:31:24]
  assign io_mem2wb_bits_exedata_aluResult = exe2mem_exedata_aluResult;	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:31:24]
  assign io_mem2wb_bits_memdata_memData = _memReadWrap_io_readData;	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:43:27]
  assign io_mem2global_forward_enable = (|exe2mem_iddata_control_wbOp) & wbValid;	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:21:27, :31:24, :59:{49,73}]
  assign io_mem2global_forward_wAddr = exe2mem_ifdata_inst[11:7];	// @[<stdin>:821:10, src/src/core/MEM/MEM.scala:31:24, :60:39]
  assign io_mem2global_forward_wData =
    exe2mem_iddata_control_wbOp == 2'h1
      ? exe2mem_exedata_aluResult
      : exe2mem_iddata_control_wbOp == 2'h2 ? _memReadWrap_io_readData : 32'h0;	// @[<stdin>:821:10, src/main/scala/chisel3/util/Mux.scala:141:16, src/src/core/MEM/MEM.scala:31:{24,37}, :43:27, :64:21, :65:21]
endmodule

module WB(	// @[<stdin>:907:10]
  input         clock,	// @[<stdin>:908:11]
                reset,	// @[<stdin>:909:11]
  ...
  always @(posedge clock) begin	// @[<stdin>:908:11]
    if (reset) begin	// @[<stdin>:908:11]
  ...
  `ifdef ENABLE_INITIAL_REG_	// @[<stdin>:907:10]
    `ifdef FIRRTL_BEFORE_INITIAL	// @[<stdin>:907:10]
      `FIRRTL_BEFORE_INITIAL	// @[<stdin>:907:10]
    `endif // FIRRTL_BEFORE_INITIAL
    logic [31:0] _RANDOM[0:8];	// @[<stdin>:907:10]
    initial begin	// @[<stdin>:907:10]
      `ifdef INIT_RANDOM_PROLOG_	// @[<stdin>:907:10]
        `INIT_RANDOM_PROLOG_	// @[<stdin>:907:10]
      `endif // INIT_RANDOM_PROLOG_
      `ifdef RANDOMIZE_REG_INIT	// @[<stdin>:907:10]
        for (logic [3:0] i = 4'h0; i < 4'h9; i += 4'h1) begin
          _RANDOM[i] = `RANDOM;	// @[<stdin>:907:10]
        end	// @[<stdin>:907:10]
        wbValid = _RANDOM[4'h0][0];	// @[<stdin>:907:10, src/src/core/WB/WB.scala:18:26]
        mem2wb_ifdata_pc = {_RANDOM[4'h0][31:1], _RANDOM[4'h1][0]};	// @[<stdin>:907:10, src/src/core/WB/WB.scala:18:26, :25:23]
        mem2wb_ifdata_inst = {_RANDOM[4'h1][31:1], _RANDOM[4'h2][0]};	// @[<stdin>:907:10, src/src/core/WB/WB.scala:25:23]
        mem2wb_iddata_control_wbOp = _RANDOM[4'h2][22:21];	// @[<stdin>:907:10, src/src/core/WB/WB.scala:25:23]
        mem2wb_iddata_control_halt = _RANDOM[4'h2][23];	// @[<stdin>:907:10, src/src/core/WB/WB.scala:25:23]
        mem2wb_exedata_aluResult = {_RANDOM[4'h5][31:24], _RANDOM[4'h6][23:0]};	// @[<stdin>:907:10, src/src/core/WB/WB.scala:25:23]
        mem2wb_memdata_memData = {_RANDOM[4'h6][31:24], _RANDOM[4'h7][23:0]};	// @[<stdin>:907:10, src/src/core/WB/WB.scala:25:23]
        io_wb2global_debug_inst_REG = {_RANDOM[4'h7][31:24], _RANDOM[4'h8][23:0]};	// @[<stdin>:907:10, src/src/core/WB/WB.scala:25:23, :52:38]
        io_wb2global_debug_halt_REG = _RANDOM[4'h8][24];	// @[<stdin>:907:10, src/src/core/WB/WB.scala:52:38, :53:38]
      `endif // RANDOMIZE_REG_INIT
    end // initial
    `ifdef FIRRTL_AFTER_INITIAL	// @[<stdin>:907:10]
      `FIRRTL_AFTER_INITIAL	// @[<stdin>:907:10]
    `endif // FIRRTL_AFTER_INITIAL
  `endif // ENABLE_INITIAL_REG_
  assign io_wb2global_debug_valid = wbValid;	// @[<stdin>:907:10, src/src/core/WB/WB.scala:18:26]
  assign io_wb2global_debug_pc = mem2wb_ifdata_pc;	// @[<stdin>:907:10, src/src/core/WB/WB.scala:25:23]
  assign io_wb2global_debug_inst = io_wb2global_debug_inst_REG;	// @[<stdin>:907:10, src/src/core/WB/WB.scala:52:38]
  assign io_wb2global_debug_halt = io_wb2global_debug_halt_REG;	// @[<stdin>:907:10, src/src/core/WB/WB.scala:53:38]
  assign io_wb2global_writeBack_enable = wbValid & (|mem2wb_iddata_control_wbOp);	// @[<stdin>:907:10, src/src/core/WB/WB.scala:18:26, :25:23, :40:{44,60}]
  assign io_wb2global_writeBack_wAddr = mem2wb_ifdata_inst[11:7];	// @[<stdin>:907:10, src/src/core/WB/WB.scala:25:23, :35:23]
  assign io_wb2global_writeBack_wData =
    mem2wb_iddata_control_wbOp == 2'h1
      ? mem2wb_exedata_aluResult
      : mem2wb_iddata_control_wbOp == 2'h2 ? mem2wb_memdata_memData : 32'h0;	// @[<stdin>:907:10, src/main/scala/chisel3/util/Mux.scala:141:16, src/src/core/WB/WB.scala:25:{23,36}, :45:21, :46:21]
endmodule

module CPUTop(	// @[<stdin>:982:10]
  input         clock,	// @[<stdin>:983:11]
                reset,	// @[<stdin>:984:11]
  ...
  assign io_inst_writeEn = 1'h0;	// @[<stdin>:982:10, src/src/core/CPUTop.scala:28:26]
  assign io_inst_writeData = 32'h0;	// @[<stdin>:982:10, src/src/core/CPUTop.scala:28:26, :30:26]
  assign io_inst_mark = 4'hF;	// @[<stdin>:982:10, src/src/core/CPUTop.scala:28:26]
  assign io_debug_regs_0 = 32'h0;	// @[<stdin>:982:10, src/src/core/CPUTop.scala:28:26, :30:26]
endmodule

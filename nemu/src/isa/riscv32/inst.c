...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)             \
    ...
    INSTPAT("0100000 ????? ????? 101 ????? 0110011", sra, R, R(rd) = (sword_t)src1 >> BITS(src2, 4, 0));

    INSTPAT("??????? ????? ????? 000 ????? 0010011", addi, I, R(rd) = src1 + (sword_t)imm);
    ...
    INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall, N, NEMUTRAP(s->pc, R(10)));   // TODO
    INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak, N, NEMUTRAP(s->pc, R(10)));  // R(10) is $a0
    // ----------------------- M ----------------------------------
    INSTPAT("0000001 ????? ????? 000 ????? 0110011", mul, R, R(rd) = ((int64_t)(sword_t)src1 * (int64_t)(sword_t)src2));
    INSTPAT("0000001 ????? ????? 001 ????? 0110011", mulh, R, R(rd) = ((int64_t)(sword_t)src1 * (int64_t)(sword_t)src2) >> 32);
    INSTPAT("0000001 ????? ????? 010 ????? 0110011", mulsu, R, R(rd) = ((int64_t)(sword_t)src1 * (uint64_t)src2) >> 32);

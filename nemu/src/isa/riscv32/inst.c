...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)             \
    ...
    INSTPAT("0000000 ????? ????? 010 ????? 0110011", slt, R, R(rd) = src1 < src2 ? 1 : 0);
    INSTPAT("0000000 ????? ????? 011 ????? 0110011", sltu, R, R(rd) = (uint32_t)src1 < (uint32_t)src2 ? 1 : 0);
    INSTPAT("0000000 ????? ????? 101 ????? 0110011", srl, R, R(rd) = (uint32_t)src1 >> src2);
    INSTPAT("0100000 ????? ????? 101 ????? 0110011", sra, R, R(rd) = src1 >> src2);
    ...
    INSTPAT("??????? ????? ????? 011 ????? 00000 11", lbu, I, R(rd) = Mr(src1 + imm, 1));
    INSTPAT("??????? ????? ????? 100 ????? 00000 11", lhu, I, R(rd) = Mr(src1 + imm, 2));

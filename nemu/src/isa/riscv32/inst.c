...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)             \
    ...
    INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq, B, if (src1 == src2) s->pc += imm);
    INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne, B, if (src1 != src2) s->pc += imm);
    INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt, B, if (src1 < src2) s->pc += imm);
    INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge, B, if (src1 >= src2) s->pc += imm);
    INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu, B, if ((uint32_t)src1 < (uint32_t)src2) s->pc += imm);
    INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu, B, if ((uint32_t)src1 >= (uint32_t)src2) s->pc += imm);
